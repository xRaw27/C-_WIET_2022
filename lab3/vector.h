...
{
public:
    explicit Vector(size_t capacity=0) : size_{0}, capacity_{capacity}, data_{new Fraction[capacity]} {}
    Vector (const Vector&);
    ...
    [[nodiscard]] const Fraction* data() const { return data_; }

    void push_back(Fraction);

    Fraction& operator[](std::size_t);
